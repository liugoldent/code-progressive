...
1. 需要最新的佈局資訊
clientWidth 屬性返回的是元素的內容區域寬度（包含內邊距但不包括邊框、滾動條與外邊距）。當 DOM 或 CSS 發生改變時，瀏覽器通常會延遲計算，批次處理這些變化以提高效能。但如果在這個過程中你讀取 clientWidth，瀏覽器必須保證返回的是最新的數值，就會立即進行佈局計算，這就是所謂的「強制同步佈局」（forced synchronous layout）。
