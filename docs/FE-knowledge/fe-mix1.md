...
## 說明display:none跟visibility: hidden的差異
### display: none：
...
* 主要判別：RWD為一個CSS，透過`@media`切換版面呈現
...
* 主要判別：AWD為多個CSS，會預先寫好版面大小的呈現，有點客製化的感覺
...
## 說明如何理解javascript的this
### 全局範圍中的 this
```js
console.log(this); // 在瀏覽器中，輸出的是 window 對象，在 Node.js 中，輸出的是 global 對象
```
### 函數中的 this：
```js
function greet() {
...
const obj = {
  foo: "bar",
  showFoo: () => {
    console.log(this.foo); // this 會參考到外部的全局範圍，輸出 undefined
  }
};
...
const obj = {
  ...
  showFoo: function() {
    setTimeout(() => {
      console.log(this.foo); // this 會參考到 obj 對象，輸出 "bar"
    }, 1000);
  }
};
...
document.getElementById("myButton").addEventListener("click", function() {
  console.log(this); // this 會參考到被點擊的按鈕元素
});
</script>
```
### 建構函數中的 this：
