...
延伸閱讀：[JavaScript / TypeScript 面試重點](./04-javascript-typescript.md)與[效能 / 測試 / 前端系統設計](./07-quality-performance-system.md)。
...
完整設計題可看 [Realtime Socket Governance](./10-realtime-socket-governance.md)。
...
練習順序使用 [React 基礎：Vue 對照版](./01-react-basics-vue-comparison.md) → [React 反射訓練](./02-react-reflex-drills.md) → [React 現場實戰題系列](./13-react-live-interview-series.md) → [React / Redux / React Query](./05-react-state-data.md)。
