...
var KthLargest = function(k, nums) {
  ...
  KthLargest.prototype.heapifyUp = function(index) {
    ...
    if (parent >= 0 && this.heap[parent] > this.heap[index]) { // 如果父節點比當前節點大，交換它們的位置並繼續向上堆化
  ...
  KthLargest.prototype.heapifyDown = function(index) {
    ...
    letsmallest = index; // 假設當前節點為最小值
    if (left < this.heap.length && this.heap[left] < this.heap[smallest]) { // 如果左子節點比當前節點小，更新最小值索引
      smallest = left;
    }
    if (right < this.heap.length && this.heap[right] < this.heap[smallest]) { // 如果右子節點比當前節點小，更新最小值索引
      smallest = right;
    }
    if (smallest !== index) { // 如果最小值索引不等於當前節點索引，交換它們的位置並繼續向下堆化
